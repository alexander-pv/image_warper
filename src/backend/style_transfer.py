...
class StyleTransfer:
    ...
    def stylize(self, content_image: np.ndarray, style_images: np.ndarray, *,
                style_weights=None,
        ...
        content_image = Image.fromarray(content_image).convert('RGB')
        style_images = [Image.fromarray(x).convert('RGB') for x in style_images]
        ...
        for scale, its in tqdm.tqdm(scales):
...
if __name__ == '__main__':
    ...
    img1 = Image.open(os.path.join(root, '..', 'tests', 'pics', 'castle.png')).convert('RGB')
    img2 = Image.open(os.path.join(root, '..', 'tests', 'pics', 'fire.png')).convert('RGB')
    st_transfer = StyleTransfer(devices=['cuda'], pooling='max')
    st_transfer.stylize(img1, [img2], iterations=10)
    st_transfer.get_image().save('out.png')
